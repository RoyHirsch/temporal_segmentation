...
parser.add_argument('--use_rw', default=False, action='store_true')
...
if args.use_rw:
    bz_stages = '/random_walk' + time_data
else:
...
if args.action == "train":
    batch_gen = BatchGenerator(num_classes, actions_dict, args.dataset, gt_path, features_path, sample_rate)
    batch_gen.read_data(vid_list_file)

    # Train the model
    if args.use_rw:
        print('Use RW preds for second stage of training')
    print('Start training')
    trainer.train(model_dir, batch_gen, num_epochs=num_epochs, batch_size=bz, learning_rate=lr, device=device, use_rw=args.use_rw)
