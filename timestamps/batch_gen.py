...
class BatchGenerator(object):
    def __init__(self, num_classes, actions_dict, dataset_name, gt_path, features_path, sample_rate):
        ...
        with open('/home/jupyter/data/'+dataset_name+'/rw_preds.pickle', 'rb') as f:
            self.rw_preds = pickle.load(f)
    ...
    def get_labels(self, batch_size, pred):
        batch = self.list_of_examples[self.index - batch_size:self.index]
        num_video, _, max_frames = pred.size()
        target_tensor = torch.ones(num_video, max_frames, dtype=torch.long) * (-100)
        for b, vid in enumerate(batch):
            labels = self.gt[vid]
            target_tensor[b, :len(labels)] = torch.tensor(labels, dtype=torch.long)
